...
A physical scoreboard for a 4-person mini-league inside a 14-team ESPN Fantasy Football league. A Raspberry Pi Pico W will drive an LED matrix display, while a Python backend hosted on Oracle Cloud handles ESPN authentication, data polling, and parsing.
...
ESPN session cookies are kept server-side only and are never committed to this repo.
